...
import { CreateCustomerInput, createCustomerSchema } from "../schemas/customer-schema"
import {
...
export function getCustomers(request: Request): Response {
    ...
    if (query.filters.dateFrom) {
        result = result.filter(
            (customer) =>
                customer.lastContactDate >= query.filters.dateFrom!
        )
    }
    ...
    if (query.filters.dateTo) {
        result = result.filter(
            (customer) =>
                customer.lastContactDate <= query.filters.dateTo!
        )
    }
...
export async function deleteCustomer(
  ...
  const [deletedCustomer] = customers.splice(
    customerIndex,
    1
  )
